...
runtime in order to build packer with the Ucloud plugin.
...
## Setting up Ucloud plugin for dev

With Go installed, you can already `go get` the Ucloud plugin and `make dev` in
...
1. Download the Ucloud plugin source (and its dependencies) by running
   `go get github.com/hashicorp/packer-plugin-ucloud`. This will download the source to
   `$GOPATH/src/github.com/hashicorp/packer-plugin-ucloud`.

2. When working on the Ucloud plugin, first `cd $GOPATH/src/github.com/hashicorp/packer-plugin-ucloud`
   so you can run `make dev` and easily access other files. `make dev` will build the packer-plugin-ucloud binary and install it under `$HOME/.packer.d/plugins/`.

3. Make your changes to the Ucloud plugin source. You can run `make dev` to build and install locally, and `make test` to run unit tests.
   Any compilation errors will be shown when the binaries are rebuilding. If you don't have `make` you can simply run `go build -o packer-plugin-ucloud` from the project root, and `mv packer-plugin-ucloud ~/.packer.d/plugins/packer-plugin-ucloud` to install the plugin.

4. After building the Ucloud plugin successfully, use the latest version of Packer to build a machine and verify your changes. In the [example folder](https://github.com/hashicorp/packer-plugin-ucloud/blob/main/example) we provide a basic template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous step.
...
need to [fork the Ucloud plugin](https://github.com/hashicorp/packer-plugin-ucloud#fork-destination-box), push your
...
# Develop a patch.
git push https://github.com/myuser/packer-plugin-ucloud f-my-feature
```
...
3. One of Ucloud plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the Ucloud plugin
project. After following the steps in "Setting up Go to work on the Ucloud plugin":

1. Navigate to the code:

   `cd $GOPATH/src/github.com/hashicorp/packer-plugin-ucloud`
...
2. Add the remote by running:
   ...
   `git remote add myuser https://github.com/myuser/packer-plugin-ucloud.git`
...
Please only apply the minimal vendor changes to get your PR to work. The Ucloud plugin
...
If the Ucloud Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
